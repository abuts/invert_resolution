...
class br(object):
    def __init__(self):
        ...
        # the id of current job used to redirect logs into it
    ...
    def run_build(self,target,buildType=None,cleanBuildAttempted=False,env=None):
        """
        ...
        @param cleanBuildAttempted -- if true, the clean build is was attempted (not to try to rebuild the same build after errors)
        @param env        -- the environmental variables necessary to run msbuild/mvc -- if none, the OS has to be set up to find MSBuild & mvc from command line
        ...
        print "-----------------------------------------------------------------------------------------------------------"
        print "-----------------------------------------------------------------------------------------------------------"
        print "--------      FINISHED BUILD : ",buildType,"        --------------------------------------------------------"
        print "-----------------------------------------------------------------------------------------------------------"
        print "-----------------------------------------------------------------------------------------------------------"
        print "-----------------------------------------------------------------------------------------------------------"
        return err
    ...
    @staticmethod
    def check_deletion(build_path):
        """ Check if Mantid within the path specified can be deleted """ 

        mantid = os.path.join(build_path,'MantidPlot.exe');
        if os.path.exists(mantid):
            try:
                os.remove(mantid)
            except:
                return False;
        else:
            # assume can delete if file does not exist
            pass
        return True;



    def build_project(self,env,repo_path,build_path,first_build=True,short=False,build_all_clean=False,buildType='Release',build_single_clean=False):
        ...
        can_delete = False;
        if build_all_clean or build_single_clean:
            can_delete = self.check_deletion(build_flavour_path);
            if not(can_delete):
               errMessage="Can not clean the project of type  :"+buildType;
        ...
        cleanBuildAttempted = build_single_clean;
        if build_exists:
            if build_all_clean:
                ...
                if not(os.path.exists(build_path)):
                    os.mkdir(build_path);
            if build_single_clean:
                build_flavour_exists = os.path.exists(build_flavour_path);
                if build_flavour_exists:
                    print "Clean build: removing existing project flavor from: {0}".format(build_flavour_path);
                    shutil.rmtree(build_flavour_path,True)
                    print "           : finished removing existing project flavor ------------";
        else:
            cleanBuildAttempted = True;
            if not(os.path.exists(build_path)):
                os.mkdir(build_path);
        ...
        if short :
            ...
            for dir,proj_name in self._MANTID_short.iteritems():
                ...
                err=self.run_build(proj_name,buildType,cleanBuildAttempted,env);
                if err>0:
...
#
    def find_cur_branch_id(self):
        ...
        if old style branch or number before / for new style branch
        ...
        try:
            id = int(filter(str.isdigit, result))
            result = str(id)
        except ValueError:
    ...
    def cron_job(self,job_description_file,dry_run=False):
        ...
        for job in jobs :
            ...
            if len(err) != 0:
                    log_head='+'.ljust(len(log_head))
                    self.append_log("{0}, Can not prepare source branch: {1}, skipping this job\n".format(log_head,err))
                    continue
